...
struct Test {
	...
	pub a: u32,
	#[special(from_type(String))]
	#[parse(async_custom(parse_two, [&tmp_2_a]))]
	pub b: Option<u32>,
	#[special(from_type(String))]
	#[parse(async_custom(parse_three, [&tmp_2_a, &tmp_2_b]))]
	pub c: Option<u32>,
}

async fn parse(value: String, _field_name: &str) -> ParseResult<u32> {
	(value.parse::<u32>().unwrap_or(0), None)
}

async fn parse_two(value: String, _field_name: &str, extra_arg: &Option<u32>) -> ParseResult<u32> {
	(extra_arg.unwrap_or(value.parse::<u32>().unwrap_or(0)), None)
}

async fn parse_three(_: String, field_name: &str, a: &Option<u32>, b: &Option<u32>) -> ParseResult<u32> {
	match (a, b) {
		(_, None) => (
			0,
