...
pub struct FieldContentTypeArgs {
	pub pattern: Option<LitStr>,
	pub code: LitStr,
	pub message: LitStr,
}

impl Default for FieldContentTypeArgs {
	fn default() -> Self {
...
impl ArgParser for FieldContentTypeArgs {
...
pub fn create_field_content_type(
	input: ParseStream,
	field: &mut FieldAttributes,
	imports: &RefCell<ImportsSet>,
) -> TokenStream {
	imports.borrow_mut().add(Import::ValidationFunction(
		"field_data::validate_field_content_type as validate_field_content_type_fn",
	));
	...
	let FieldContentTypeArgs { pattern, code, message } = match content {
		Ok(content) => parse_attrs(&content)
			.inspect_err(|erro| emit_error!(erro.span(), "{}", erro))
			.unwrap_or_default(),
		Err(_) => FieldContentTypeArgs::default(),
	};
	...
	if field.is_ref() {
		...
		#[rustfmt::skip]
		let result = quote! {
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = validate_field_content_type_fn(#reference, #pattern, #field_name, #code, #message) {
		...
		let result = quote! {
		  let _ref = &#reference;
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = validate_field_content_type_fn(_ref, #pattern, #field_name, #code, #message) {
        append_error(&mut errors, e, failure_mode, #field_name);
        if should_fail_fast(&errors, failure_mode, #field_name) {
     			return Err(errors);
     	  }
