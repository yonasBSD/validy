...
#[derive(Default)]
struct CustomWithContextArgs {
	function: Option<Ident>,
	params: Option<ExprArray>,
}

impl ArgParser for CustomWithContextArgs {
...
pub fn create_custom_with_context(
	...
	let CustomWithContextArgs { function, params } = match content {
		Ok(content) => parse_attrs(&content)
			...
			.unwrap_or_default(),
		Err(_) => CustomWithContextArgs::default(),
	};
	...
	if field.is_ref() {
		...
		#[rustfmt::skip]
		let result = quote! {
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = #function(#reference, #field_name, context, #(#extra_args),*) {
		...
		let result = quote! {
		  let _ref = &#reference;
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = #function(_ref, #field_name, context, #(#extra_args),*) {
        append_error(&mut errors, e, failure_mode, #field_name);
        if should_fail_fast(&errors, failure_mode, #field_name) {
     			return Err(errors);
     	  }
