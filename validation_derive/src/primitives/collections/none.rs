...
pub fn create_none(input: ParseStream, field: &FieldAttributes, imports: &RefCell<ImportsSet>) -> TokenStream {
	imports
		.borrow_mut()
		.add(Import::ValidationFunction("iter::validate_none as validate_none_fn"));

	let field_name = field.get_name();
	let reference = field.get_reference();
	let content = remove_parens(input);

	let NoneArgs { items, code, message } = match content {
	...
	quote! {
		if let Err(e) = validate_none_fn(&#reference, #items, #field_name, #code, #message) {
		  errors.push(e);
	  }
	}
}
