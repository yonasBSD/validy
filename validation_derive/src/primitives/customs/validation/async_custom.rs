...
#[derive(Default)]
pub struct AsyncCustomArgs {
	pub function: Option<Ident>,
	pub params: Option<ExprArray>,
}

impl ArgParser for AsyncCustomArgs {
...
pub fn create_async_custom(
	...
	if field.is_ref() {
		...
		#[rustfmt::skip]
		let result = quote! {
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = #function(#reference, #field_name, #(#extra_args),*).await {
		...
		let result = quote! {
		  let _ref = &#reference;
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = #function(_ref, #field_name, #(#extra_args),*).await {
			  append_error(&mut errors, e, failure_mode, #field_name);
        if should_fail_fast(&errors, failure_mode, #field_name) {
     			return Err(errors);
     	  }
