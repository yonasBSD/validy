...
pub fn create_time(input: ParseStream, field: &FieldAttributes) -> TokenStream {
	let field_name = field.get_name();
	let reference = field.get_reference();
	let content = remove_parens(input);
	let import = import_validation_functions("time::validate_time");
	...
	quote! {
	  use #import;
		if let Err(e) = validate_time(&#reference, #format, #field_name, #code, #message) {
		  errors.push(e);
	  }
	}
}
