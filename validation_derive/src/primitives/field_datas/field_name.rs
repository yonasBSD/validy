...
pub struct FieldNameArgs {
	pub pattern: Option<LitStr>,
	pub code: LitStr,
	pub message: LitStr,
}

impl Default for FieldNameArgs {
	fn default() -> Self {
		FieldNameArgs {
			pattern: None,
			code: LitStr::new("field_name", Span::call_site()),
			message: LitStr::new("invalid field name", Span::call_site()),
		}
	}
}

impl ArgParser for FieldNameArgs {
...
pub fn create_field_name(
	...
	let FieldNameArgs { pattern, code, message } = match content {
		Ok(content) => parse_attrs(&content)
			.inspect_err(|erro| emit_error!(erro.span(), "{}", erro))
			.unwrap_or_default(),
		Err(_) => FieldNameArgs::default(),
	};
	...
	if field.is_ref() {
		...
		#[rustfmt::skip]
		let result = quote! {
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = validate_field_name_fn(#reference, #pattern, #field_name, #code, #message) {
		...
		let result = quote! {
		  let _ref = &#reference;
			if can_continue(&errors, failure_mode, #field_name) && let Err(e) = validate_field_name_fn(_ref, #pattern, #field_name, #code, #message) {
        append_error(&mut errors, e, failure_mode, #field_name);
        if should_fail_fast(&errors, failure_mode, #field_name) {
     			return Err(errors);
     	  }
